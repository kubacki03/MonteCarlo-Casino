...
namespace MonteCarlo.NET.Areas.Identity.Pages.Account
{
    public class LoginModel : PageModel
    {
        ...
        public LoginModel(SignInManager<KontoUzytkownika> signInManager, ILogger<LoginModel> logger, KasynoContext context)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (!user.LockoutEnabled)
                {
                    var ban = await _context.Ban
                        .Where(b => b.KontoUzytkownikaId == user.Id)
                        .OrderByDescending(b => b.Data)
                        .FirstOrDefaultAsync();

                    ModelState.AddModelError(string.Empty, $"Your account is disabled. Reason: {ban.Przyczyna}.");
                    return Page();
                }
